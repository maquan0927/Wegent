...
3. Send task via sandbox.run_code() - returns poll_url
...
Note: This operation may take several minutes for complex tasks."""
    ...
    async def _arun(
        ...
        2. Sends task via run_code and gets poll_url
    ...
    async def _execute_task(
        ...
        try:
            ...
            # Parse poll_url from response
            try:
                poll_info = json.loads(execution.text)
                poll_url = poll_info.get("poll_url")

                if not poll_url:
                    # Might be direct result
                    logger.info("[SubAgentTool] No poll_url, treating as direct result")
                ...
                logger.info(f"[SubAgentTool] Got poll_url: {poll_url}")
                ...
                # Direct result (not JSON with poll_url)
                return self._format_success(
                    {
                        "status": "success",
                        "result": execution.text,
