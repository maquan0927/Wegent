...
async def _get_sandbox_info(sandbox_id: str) -> tuple:
    """Get sandbox info including host, port, and metadata.

    Args:
        sandbox_id: E2B sandbox UUID

    Returns:
        Tuple of (host, port, sandbox) for the container
    """
    config = get_config()
    ...
    # Get container port from sandbox
    port = sandbox.port
    if not port:
        raise HTTPException(
            status_code=503,
            detail={
                "code": "unavailable",
                "message": f"Sandbox {sandbox_id} port not available",
            },
        )

    host = config.executor.docker_host
    return host, port, sandbox
...
async def proxy_to_sandbox(sandbox_id: str, port: int, path: str, request: Request):
    ...
    try:
        ...
        host, container_host_port, sandbox = await _get_sandbox_info(sandbox_id)
        ...
        if is_subagent and path == "execute" and request.method == "POST":
            return await _proxy_subagent_execute(
                host, container_host_port, sandbox, body, request
            )

        # Regular proxy for non-subagent tasks or non-execute paths
        target_url = f"http://{host}:{container_host_port}/{path}"
...
async def _proxy_subagent_execute(
    _host: str, _port: int, sandbox, body: bytes, _request: Request
):
    ...
    we immediately return a poll_url for the client to query results.
    ...
    Returns:
        StreamingResponse with E2B standard NDJSON format containing poll_url
    """
    try:
        ...
        # Build poll_url for client to query results
        # Get base URL from environment or use default
        external_url = os.getenv("EXTERNAL_URL")
        if not external_url:
            # Fallback to localhost with default port
            server_port = os.getenv("SERVER_PORT", "8001")
            external_url = f"http://localhost:{server_port}"

        poll_url = f"{external_url}/executor-manager/sandboxes/{sandbox_id}/executions/{subtask_id}"
        ...
        result_payload = {
            ...
            "status": "pending",
            "message": "Task submitted. Poll the poll_url for results.",
        }

        logger.info(f"[SandboxProxy] Execution created, returning poll_url: {poll_url}")
