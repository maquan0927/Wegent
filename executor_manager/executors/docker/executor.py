...
class DockerExecutor(Executor):
    ...
    def _add_sandbox_env_vars(self, cmd: List[str], task: Dict[str, Any]) -> None:
        ...
        - EXECUTOR_MANAGER_CALLBACK_URL: Used by heartbeat service for heartbeat endpoint
        ...
        if sandbox_id:
            ...
            # Build heartbeat URL from callback URL
            callback_url = build_callback_url(task)
            if callback_url and "/callback" in callback_url:
                ...
                cmd.extend(["-e", f"EXECUTOR_MANAGER_CALLBACK_URL={base_url}/callback"])
