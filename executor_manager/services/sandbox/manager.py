...
class SandboxManager(metaclass=SingletonMeta):
    ...
    async def create_sandbox(
        ...
        logger.info(
            ...
            f"container={sandbox.container_name}, port={sandbox.port}"
        )

        return sandbox, None

    async def _start_sandbox_container(self, sandbox: Sandbox) -> Optional[str]:
        """Start the Docker container for a sandbox.
        ...
        # Wait for container to be ready and get port using health checker
        port = await self._health_checker.wait_for_container_ready(container_name)
        if port is None:
            return f"Container {container_name} failed to become ready"

        sandbox.set_running(port)
        self._repository.save_sandbox(sandbox)
    ...
    async def get_sandbox(
        ...
        if check_health and sandbox.port:
            is_healthy = self._health_checker.check_health_sync(sandbox.port)
            if not is_healthy:
